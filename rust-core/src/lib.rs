...
struct KiraState {
    ...
    ota_check_interval_ms: u128,
    // v39-fix: reactive engine fields
    // v39-fix: reactive engine fields
...
// v39: Automation engine structs
#[derive(Clone)]
struct AutoProfile { id: String, name: String, enabled: bool, created: u128, color: String }
#[derive(Clone)]
struct AutoMacro {
    id: String, name: String, profile_id: String, enabled: bool,
    trigger_kind: String, trigger_value: String,
    constraints: Vec<Constraint>, actions: Vec<AutoAction>,
    run_count: u64, last_run: u128, cooldown_ms: u128, created: u128,
    tags: Vec<String>,
}
#[derive(Clone)]
struct Constraint { kind: String, value: String, negate: bool }
#[derive(Clone)]
struct AutoAction { kind: String, params: String, delay_ms: u128 }
#[derive(Clone, Default)]
struct DeviceState {
    battery_pct: u8, is_charging: bool, screen_on: bool,
    wifi_ssid: String, wifi_connected: bool,
    volume_media: u8, headphones_in: bool,
    active_app: String, call_state: String, ringer_mode: String,
    airplane_mode: bool, lat: f64, lon: f64,
}
#[derive(Clone)]
struct VarEntry { name: String, value: String, kind: String, global: bool, ts: u128 }
#[derive(Clone)]
struct MacroRun { macro_id: String, macro_name: String, trigger: String, ts: u128, success: bool, actions_run: u32, error: String }
#[derive(Clone)]
struct GeoZone { id: String, name: String, lat: f64, lon: f64, radius_m: f64, inside: bool }

fn make_builtin_macros() -> Vec<AutoMacro> {
    let now = now_ms();
    vec![
        AutoMacro { id: "b_low_bat".to_string(), name: "Low Battery Alert".to_string(),
            profile_id: String::new(), enabled: true, trigger_kind: "battery_level".to_string(), trigger_value: "20".to_string(),
            constraints: vec![Constraint{kind:"not_charging".to_string(),value:String::new(),negate:false}],
            actions: vec![
                AutoAction{kind:"notify".to_string(),params:r#"{"title":"Low Battery","text":"Battery below 20%"}"#.to_string(),delay_ms:0},
                AutoAction{kind:"send_telegram".to_string(),params:r#"{"text":"[Kira] Battery low"}"#.to_string(),delay_ms:0}],
            run_count:0,last_run:0,cooldown_ms:300000,created:now,tags:vec!["battery".to_string()]},
        AutoMacro { id: "b_boot".to_string(), name: "On Boot - Start Kira".to_string(),
            profile_id: String::new(), enabled: true, trigger_kind: "device_boot".to_string(), trigger_value: String::new(),
            constraints: vec![],
            actions: vec![
                AutoAction{kind:"start_kira_service".to_string(),params:"{}".to_string(),delay_ms:2000},
                AutoAction{kind:"notify".to_string(),params:r#"{"title":"Kira","text":"Agent started."}"#.to_string(),delay_ms:3000}],
            run_count:0,last_run:0,cooldown_ms:0,created:now,tags:vec!["boot".to_string()]},
        AutoMacro { id: "b_morning".to_string(), name: "Morning Briefing 07:30".to_string(),
            profile_id: String::new(), enabled: false, trigger_kind: "time_of_day".to_string(), trigger_value: "07:30".to_string(),
            constraints: vec![],
            actions: vec![AutoAction{kind:"run_workflow".to_string(),params:r#"{"workflow":"morning_brief"}"#.to_string(),delay_ms:0}],
            run_count:0,last_run:0,cooldown_ms:82800000,created:now,tags:vec!["schedule".to_string()]},
        AutoMacro { id: "b_screen_off".to_string(), name: "Screen Off - Vibrate".to_string(),
            profile_id: String::new(), enabled: false, trigger_kind: "screen_off".to_string(), trigger_value: String::new(),
            constraints: vec![],
            actions: vec![AutoAction{kind:"set_ringer_mode".to_string(),params:r#"{"mode":"vibrate"}"#.to_string(),delay_ms:0}],
            run_count:0,last_run:0,cooldown_ms:0,created:now,tags:vec!["screen".to_string()]},
        AutoMacro { id: "b_wifi_home".to_string(), name: "Home WiFi - Full Volume".to_string(),
            profile_id: String::new(), enabled: false, trigger_kind: "wifi_connected".to_string(), trigger_value: "home".to_string(),
            constraints: vec![],
            actions: vec![AutoAction{kind:"set_volume".to_string(),params:r#"{"stream":"ring","level":100}"#.to_string(),delay_ms:0}],
            run_count:0,last_run:0,cooldown_ms:60000,created:now,tags:vec!["wifi".to_string()]},
        AutoMacro { id: "b_whatsapp".to_string(), name: "AI Process WhatsApp Notif".to_string(),
            profile_id: String::new(), enabled: false, trigger_kind: "notification".to_string(), trigger_value: "com.whatsapp".to_string(),
            constraints: vec![Constraint{kind:"screen_off".to_string(),value:String::new(),negate:false}],
            actions: vec![AutoAction{kind:"ai_process_notification".to_string(),params:r#"{"prompt":"Summarize and reply if needed"}"#.to_string(),delay_ms:500}],
            run_count:0,last_run:0,cooldown_ms:5000,created:now,tags:vec!["whatsapp".to_string(),"ai".to_string()]},
    ]
}

fn fire_automation_event(s: &mut KiraState, event_type: &str, event_data: &str) {
    let now = now_ms();
    let mut to_fire: Vec<usize> = Vec::new();
    for (i, m) in s.macros.iter().enumerate() {
        if !m.enabled { continue; }
        if m.trigger_kind != event_type { continue; }
        if m.cooldown_ms > 0 && now.saturating_sub(m.last_run) < m.cooldown_ms { continue; }
        if !m.trigger_value.is_empty() && m.trigger_value != "*"
            && !event_data.to_lowercase().contains(&m.trigger_value.to_lowercase()) { continue; }
        if !eval_constraints_auto(&m.constraints, &s.device_state, &s.variables) { continue; }
        to_fire.push(i);
    }
    for i in to_fire {
        let m = s.macros[i].clone();
        for (step, action) in m.actions.iter().enumerate() {
            let cmd = format!("{{"macro_id":"{}","step":{},"kind":"{}","params":{},"delay":{}}}",
                esc(&m.id), step, esc(&action.kind), action.params, action.delay_ms);
            s.pending_cmds.push_back((m.id.clone(), cmd));
        }
        s.macros[i].run_count += 1;
        s.macros[i].last_run = now;
        s.macro_history.push_back(MacroRun { macro_id: m.id.clone(), macro_name: m.name.clone(),
            trigger: event_type.to_string(), ts: now, success: true,
            actions_run: m.actions.len() as u32, error: String::new() });
        if s.macro_history.len() > 500 { s.macro_history.pop_front(); }
        s.event_feed.push_back(EventFeedEntry {
            event: format!("macro:{}", esc(&m.name)),
            data: format!("{{"trigger":"{}"}}", event_type), ts: now });
    }
}

fn eval_constraints_auto(cs: &[Constraint], dev: &DeviceState, vars: &HashMap<String,VarEntry>) -> bool {
    for c in cs {
        let pass = match c.kind.as_str() {
            "battery_above"  => dev.battery_pct as u32 > c.value.parse().unwrap_or(0),
            "battery_below"  => (dev.battery_pct as u32) < c.value.parse().unwrap_or(100),
            "screen_on"      => dev.screen_on,
            "screen_off"     => !dev.screen_on,
            "wifi_connected" => dev.wifi_connected,
            "charging"       => dev.is_charging,
            "not_charging"   => !dev.is_charging,
            "headphones_in"  => dev.headphones_in,
            "silent_mode"    => dev.ringer_mode == "silent",
            "var_equals"     => { let p:Vec<&str>=c.value.splitn(2,'=').collect(); p.len()==2 && vars.get(p[0]).map(|v|v.value==p[1]).unwrap_or(false) },
            _ => true,
        };
        if (if c.negate { !pass } else { pass }) == false { return false; }
    }
    true
}

fn check_geo_triggers(s: &mut KiraState, lat: f64, lon: f64) {
    let mut evts: Vec<(String,String)> = Vec::new();
    for z in s.geo_zones.iter_mut() {
        let dlat = (z.lat-lat).to_radians();
        let dlon = (z.lon-lon).to_radians();
        let a = (dlat/2.0).sin().powi(2)+lat.to_radians().cos()*z.lat.to_radians().cos()*(dlon/2.0).sin().powi(2);
        let dist = 6371000.0f64 * 2.0*a.sqrt().atan2((1.0-a).sqrt());
        let was = z.inside; z.inside = dist <= z.radius_m;
        if !was && z.inside { evts.push(("location_entered".to_string(), format!("{{"zone":"{}","dist":{:.0}}}", esc(&z.name), dist))); }
        else if was && !z.inside { evts.push(("location_exited".to_string(), format!("{{"zone":"{}","dist":{:.0}}}", esc(&z.name), dist))); }
    }
    for (e,d) in evts { fire_automation_event(s, &e, &d); }
}


// v39-fix: Missing structs
#[derive(Clone, Default)]
struct RxSubscription {
    id: String, event_pattern: String, callback_url: String,
    active: bool, created: u128, fire_count: u64,
}
#[derive(Clone, Default)]
struct CompositeTrigger {
    id: String, name: String, operator: String,
    sub_trigger_ids: Vec<String>, enabled: bool, fire_count: u64,
}
#[derive(Clone, Default)]
struct StateMachine {
    id: String, name: String, states: Vec<String>,
    current_state: String, enabled: bool,
    transitions: Vec<StateTransition>,
}
#[derive(Clone, Default)]
struct StateTransition {
    from_state: String, to_state: String,
    trigger_event: String, action: String,
}
#[derive(Clone, Default)]
struct ContextZone {
    id: String, name: String, trigger_kind: String,
    trigger_value: String, active: bool, priority: u32,
}
...
fn route_http(method: &str, path: &str, body: &str) -> String {
    ...
    match (method, path_clean) {
        ...
        // OTA update engine
        ("GET", "/ota/status")    => { let s=STATE.lock().unwrap(); format!("{{\"current_version\":\"{}\",\"latest_version\":\"{}\",\"update_available\":{},\"last_check\":{},\"changelog\":\"{}\",\"download_url\":\"{}\"}}",esc(&s.ota_current_version),esc(&s.ota_latest_version),s.ota_update_available,s.ota_last_check,esc(&s.ota_changelog),esc(&s.ota_download_url)) }
        ("POST", "/ota/set_version") => { let v=extract_json_str(body,"version").unwrap_or_default(); if !v.is_empty(){STATE.lock().unwrap().ota_current_version=v.clone();} r#"{"ok":true}"#.to_string() }
        ("POST", "/ota/check")     => { let latest=extract_json_str(body,"latest").unwrap_or_default(); let url=extract_json_str(body,"download_url").unwrap_or_default(); let log=extract_json_str(body,"changelog").unwrap_or_default(); let mut s=STATE.lock().unwrap(); s.ota_latest_version=latest.clone(); s.ota_download_url=url; s.ota_changelog=log; s.ota_update_available= !latest.is_empty() && latest!=s.ota_current_version; s.ota_last_check=now_ms(); let avail=s.ota_update_available; format!("{{\"ok\":true,\"update_available\":{}}}", avail) }

        // v39 Automation engine
        ("GET",  "/automation/macros")  => { let s=STATE.lock().unwrap();let v:Vec<String>=s.macros.iter().map(|m|format!("{{\"id\":\"{}\",\"name\":\"{}\",\"enabled\":{},\"trigger_kind\":\"{}\",\"trigger_value\":\"{}\",\"run_count\":{},\"actions\":{},\"tags\":[{}]}}",esc(&m.id),esc(&m.name),m.enabled,esc(&m.trigger_kind),esc(&m.trigger_value),m.run_count,m.actions.len(),m.tags.iter().map(|t|format!("\"{}\"",esc(t))).collect::<Vec<_>>().join(","))).collect();format!("[{}]",v.join(",")) }
        ("POST", "/automation/macros")  => { let id=extract_json_str(body,"id").unwrap_or_else(gen_id);let name=extract_json_str(body,"name").unwrap_or_else(||"New Macro".to_string());let tk=extract_json_str(body,"trigger_kind").unwrap_or_default();let tv=extract_json_str(body,"trigger_value").unwrap_or_default();let cool=extract_json_str(body,"cooldown_ms").and_then(|s|s.parse().ok()).unwrap_or(0u128);let m=AutoMacro{id,name,profile_id:String::new(),enabled:true,trigger_kind:tk,trigger_value:tv,constraints:vec![],actions:vec![],run_count:0,last_run:0,cooldown_ms:cool,created:now_ms(),tags:vec![]};let mut s=STATE.lock().unwrap();s.macros.retain(|x|x.id!=m.id);s.macros.push(m);r#"{"ok":true}"#.to_string() }
        ("DELETE","/automation/macros") => { let id=extract_json_str(body,"id").unwrap_or_default();STATE.lock().unwrap().macros.retain(|m|m.id!=id);r#"{"ok":true}"#.to_string() }
        ("POST", "/automation/macros/toggle") => { let id=extract_json_str(body,"id").unwrap_or_default();let mut s=STATE.lock().unwrap();for m in s.macros.iter_mut(){if m.id==id{m.enabled= !m.enabled;return format!("{{\"ok\":true,\"enabled\":{}}}", m.enabled);}}r#"{"ok":false}"#.to_string() }
        ("GET",  "/automation/profiles") => { let s=STATE.lock().unwrap();let v:Vec<String>=s.profiles.iter().map(|p|format!("{{\"id\":\"{}\",\"name\":\"{}\",\"enabled\":{},\"color\":\"{}\"}}",esc(&p.id),esc(&p.name),p.enabled,esc(&p.color))).collect();format!("[{}]",v.join(",")) }
        ("POST", "/automation/profiles") => { let id=extract_json_str(body,"id").unwrap_or_else(gen_id);let name=extract_json_str(body,"name").unwrap_or_default();let color=extract_json_str(body,"color").unwrap_or_else(||"#DC143C".to_string());let p=AutoProfile{id,name,enabled:true,created:now_ms(),color};let mut s=STATE.lock().unwrap();s.profiles.retain(|x|x.id!=p.id);s.profiles.push(p);r#"{"ok":true}"#.to_string() }
        ("POST", "/automation/event")   => { let et=extract_json_str(body,"event").unwrap_or_default();let ed=extract_json_str(body,"data").unwrap_or_default();let mut s=STATE.lock().unwrap();fire_automation_event(&mut s,&et,&ed);r#"{"ok":true}"#.to_string() }
        ("POST", "/automation/device")  => { let mut s=STATE.lock().unwrap();if let Some(v)=extract_json_str(body,"battery_pct"){s.device_state.battery_pct=v.parse().unwrap_or(100);s.battery_pct=s.device_state.battery_pct as i32;}if let Some(v)=extract_json_str(body,"is_charging"){s.device_state.is_charging=v=="true";}if let Some(v)=extract_json_str(body,"screen_on"){s.device_state.screen_on=v=="true";}if let Some(v)=extract_json_str(body,"wifi_ssid"){s.device_state.wifi_ssid=v;}if let Some(v)=extract_json_str(body,"active_app"){s.device_state.active_app=v;}if let Some(v)=extract_json_str(body,"ringer_mode"){s.device_state.ringer_mode=v;}r#"{"ok":true}"#.to_string() }
        ("GET",  "/automation/history") => { let s=STATE.lock().unwrap();let skip=s.macro_history.len().saturating_sub(50);let v:Vec<String>=s.macro_history.iter().skip(skip).map(|r|format!("{{\"id\":\"{}\",\"name\":\"{}\",\"trigger\":\"{}\",\"ts\":{},\"ok\":{},\"steps\":{}}}",esc(&r.macro_id),esc(&r.macro_name),esc(&r.trigger),r.ts,r.success,r.actions_run)).collect();format!("[{}]",v.join(",")) }
        ("GET",  "/automation/variables") => { let s=STATE.lock().unwrap();let v:Vec<String>=s.variables.values().map(|e|format!("{{\"name\":\"{}\",\"value\":\"{}\",\"kind\":\"{}\"}}",esc(&e.name),esc(&e.value),esc(&e.kind))).collect();format!("[{}]",v.join(",")) }
        ("POST", "/automation/variables") => { let name=extract_json_str(body,"name").unwrap_or_default();let value=extract_json_str(body,"value").unwrap_or_default();let kind=extract_json_str(body,"kind").unwrap_or_else(||"string".to_string());if !name.is_empty(){let e=VarEntry{name:name.clone(),value,kind,global:true,ts:now_ms()};STATE.lock().unwrap().variables.insert(name,e);}r#"{"ok":true}"#.to_string() }
        ("GET",  "/automation/zones")   => { let s=STATE.lock().unwrap();let v:Vec<String>=s.geo_zones.iter().map(|z|format!("{{\"id\":\"{}\",\"name\":\"{}\",\"inside\":{},\"radius\":{:.0}}}",esc(&z.id),esc(&z.name),z.inside,z.radius_m)).collect();format!("[{}]",v.join(",")) }
        ("POST", "/automation/zones")   => { let id=extract_json_str(body,"id").unwrap_or_else(gen_id);let name=extract_json_str(body,"name").unwrap_or_default();let lat=extract_json_str(body,"lat").and_then(|s|s.parse().ok()).unwrap_or(0.0f64);let lon=extract_json_str(body,"lon").and_then(|s|s.parse().ok()).unwrap_or(0.0f64);let rad=extract_json_str(body,"radius_m").and_then(|s|s.parse().ok()).unwrap_or(200.0f64);let z=GeoZone{id:id.clone(),name,lat,lon,radius_m:rad,inside:false};let mut s=STATE.lock().unwrap();s.geo_zones.retain(|x|x.id!=id);s.geo_zones.push(z);r#"{"ok":true}"#.to_string() }
        ("POST", "/automation/webhook/:id") => { let wid=path.trim_start_matches("/automation/webhook/").to_string();let mut s=STATE.lock().unwrap();fire_automation_event(&mut s,"webhook",&format!("{{\"webhook_id\":\"{}\",\"body\":\"{}\"}}", esc(&wid),esc(body)));r#"{"ok":true}"#.to_string() }
